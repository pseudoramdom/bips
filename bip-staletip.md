...
- Nodes SHOULD respect the peer's `prefers_blocks` setting. If a peer sets
  ...
  obtained the relevant block data.
...
Announcements to peers that prefer block data are deferred until the sender has
obtained that data, with no time-based fallback to a header-only announcement.
Such a fallback would require either advertising the same tip to the same peer
twice — once without and once with block availability — or leaving the peer
uninformed if the block data arrives after the fallback, and it would introduce
an implementation-defined timeout whose value is observable in relay timing.
Deferral is not expected to harm propagation: stale tips are relayed by many
nodes, so a peer that prefers block data will in all probability also receive
the header promptly from another of its peers that relays announcements without
